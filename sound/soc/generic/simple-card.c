				     ...
				     struct simple_card_data *priv)
{
	...
	printk("!!!!8__%s__oooooooooooooooooook\n",__func__);
	return 0;
}
...
static int asoc_simple_card_probe(struct platform_device *pdev)
{
	...
	if (!priv->dai_props)
	{
		...
		printk("@@@priv dai props!\n");
		return -ENOMEM;
	}
	if (np && of_device_is_available(np)) {

		printk(" np  device available \n");
		...
		if (!cinfo) {
			dev_err(dev, "no info for asoc-simple-card\n");
		        ...
		        printk("@@@ no infor for asoc simple card !!\n");
			return -EINVAL;
		}
